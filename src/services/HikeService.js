export default class HikeService {
  ...
  // function to calculate the total distance, max and min temps,
  // total time of the hike, denivele, accumulated rain, max wind, etc
  async calculateHikeStats(hikeData) {
    ...
    for (let i = 0; i < hikeData.trackPoints.length - 1; i++) {
      const a = hikeData.trackPoints[i]
      const b = hikeData.trackPoints[i + 1]
      stats.totalTime += b.time - a.time
